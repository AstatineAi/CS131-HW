...
let insn_flow ((u, i) : uid * insn) (d : fact) : fact =
  ...
  | Bitcast (_, Id v, _) | Gep (_, Id v, _) ->
    d |> UidM.add u SymPtr.MayAlias |> UidM.add v SymPtr.MayAlias
...
module Fact = struct
  ...
  *)
  let combine (ds : fact list) : fact =
    let combine_ptr (p1 : SymPtr.t) (p2 : SymPtr.t) : SymPtr.t =
      match p1, p2 with
      | Unique, Unique -> Unique
      | MayAlias, _ | _, MayAlias -> MayAlias
      | UndefAlias, ptr | ptr, UndefAlias -> ptr
    in
    let merge_ptr (u : uid) (p1 : SymPtr.t option) (p2 : SymPtr.t option)
      : SymPtr.t option
      =
      match p1, p2 with
      | Some p1, Some p2 -> Some (combine_ptr p1 p2)
      | Some p1, None -> Some p1
      | None, Some p2 -> Some p2
      | None, None -> None
    in
    List.fold_left (fun acc d -> UidM.merge merge_ptr acc d) UidM.empty ds
  ;;
end
